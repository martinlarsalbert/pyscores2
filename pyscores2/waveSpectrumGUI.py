...
class waveSpectrumWidget(QtGui.QDockWidget):
	def __init__(self,parent=None):
		...
		self.actionAddSpectrum = QtGui.QAction(QtGui.QIcon('plus.png'),"Add Spectrum",self)	
		...
		self.actionDeleteSpectrum = QtGui.QAction(QtGui.QIcon('exit.png'),"Delete Spectrum",self)	
	...
	def addSpectrum(self,newSpectrum=None):
		...
		if spectrumType == 'ITTCSpectrum':
			self.parent.scores2Indata.runOptions["ID"].setValue(3) # Note that this equals a Two parameter spectrum.
		...
		spectrumItem = self.addParent(self.spectrumsItem,0,newSpectrum.name,None)
	...
	def deleteSpectrum(self,point):
		...
		currentSpectrumIndex = self.spectrumItemDict[self.treeWidget.currentItem()]

		self.treeWidget.currentItem().parent().removeChild(self.treeWidget.currentItem())
	...
	def editSpectrum(self,point):
		...
		self.waveSpectrums[currentSpectrumIndex] = addSpectrumWindow.waveSpectrum

		self.updateSpectrumWidget(self.treeWidget.currentItem(),addSpectrumWindow.waveSpectrum)
	...
	def showIntegration(self):
		...
		currentSpectrumItem = self.treeWidget.currentItem().parent().parent().parent().parent().parent()
		...
		yLabel = "Spectrum [m2*sec]"
		xLabel = "wave frequency [rad/s]"
		title = "%s speed:%2.2f wave direction:%2.0f %s %s" % (spectrum.getTitle(),speed,waveDirection,kind,variable)

		thePlotWindow1.plot2(spectrum.frequencies,spectrum.S,result.RAOFrequencies,result.RAO,xLabel,yLabel,title)
	...
	def calculate(self):
		...
		if self.scoresFile !=None:
			...
			QtGui.QMessageBox.about(self, "Calculation error", "You have to get RAOs first. For instance by running ScoresII")
				...
				for kind in spectrum.results[speed][waveDirection].keys():
					...
					for variable in sorted(spectrum.results[speed][waveDirection][kind].keys()):
						...
						if spectrum.results[speed][waveDirection][kind][variable].significantValue:
							value = spectrum.results[speed][waveDirection][kind][variable].significantValue
						else:
							value = spectrum.results[speed][waveDirection][kind][variable].mean

						variableItem = QtGui.QTreeWidgetItem(kindItem, [variable,str(value)])
						
						self.resultVariableDict[variableItem] = {}
						self.resultVariableDict[variableItem]['speed'] = speed
						self.resultVariableDict[variableItem]['waveDirection'] = waveDirection
						self.resultVariableDict[variableItem]['kind'] = kind
						self.resultVariableDict[variableItem]['variable'] = variable
