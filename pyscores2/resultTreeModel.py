...
HORIZONTAL_HEADERS = ("","Value","Frequency","Encounter Frequency","Wave Length")
	...
	def setupModelData(self,results):
		...
		for waveResults in results:
			...
			for result in waveResults:
				...
				pickVariables = ['addedResistance','verticalPlaneResponses','lateralPlaneResponses','pointAccelerations']				
				...
				for variable in pickVariables:
					...
					for subVariable in subVariables.iterkeys():
						if type(subVariables[subVariable]) == type(np.array([]))\
							...
							item4 = TreeItem(subVariable,variables[variable] ,'', item3)	
																			
							item5 = ChildItem(' ',variables[variable] ,'', item4)
...
class ChildItem(TreeItem):
	...
	def __init__(self,text,dataContainer,header,parentItem):
		super(ChildItem,self).__init__(text,dataContainer,header,parentItem)
