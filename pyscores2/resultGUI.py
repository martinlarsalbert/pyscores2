...
class ResultWidget(QtGui.QDockWidget):
	def __init__(self,parent=None):
		...
		self.treeWidget.setHeaderLabels([' ','value','frequency','encounter frequency','wave length'])
	...
	def showResults(self,resultPath):
		...
		self.scoresFile = scorseFileParser.scorseFileClass(self.resultPath)		
		#Run bystrom correction for all added resistance raos:
		#self.scoresFile.runBystromCorrectionForAll()
		
		self.treeWidget.clear()
		self.fillTree(self.treeWidget.invisibleRootItem(),self.scoresFile.results)
	...
	def fillTree(self,parent,results):
		...
		for speed in sorted(results.iterkeys()):
			...
			for waveAngle in sorted(results[speed].iterkeys()):
				...
				item3 = self.addChild(item2, column, 'addedResistance', results[speed][waveAngle].addedResistance)
				
				if results[speed][waveAngle].lateralPlaneResponses.frequencies != None:
					item3 = self.addChild(item2, column, 'lateralPlaneResponses', results[speed][waveAngle].lateralPlaneResponses)
				
				item3 = self.addChild(item2, column, 'verticalPlaneResponses', results[speed][waveAngle].verticalPlaneResponses)
				
				item3 = self.addChild(item2, column, 'pointAccelerations', results[speed][waveAngle].pointAccelerations)
	...
	def addChild(self, parent, column, title, datas):
		...
		for variable in variables.iterkeys():
			if type(variables[variable]) == type(np.array([])) \
				...
				for value,frequency,encounterFrequency,waveLength in zip(variables[variable],variables["frequencies"],variables["encounterFrequencies"],variables["waveLengths"]):
					item3 = QtGui.QTreeWidgetItem(item2, [' ',str(value),str(frequency),str(encounterFrequency),str(waveLength)])
	...
	def plot(self):
		...
		yLabel = "%s %s" % (self.treeWidget.currentItem().parent().data(0,0).toString(),self.treeWidget.currentItem().data(0,0).toString())
		xLabel = "wave frequency [rad/s]"
		title = "%s %s %s" % (self.treeWidget.currentItem().parent().data(0,0).toString(),self.treeWidget.currentItem().parent().parent().data(0,0).toString(),self.treeWidget.currentItem().parent().parent().parent().data(0,0).toString())

		thePlotWindow.plot(frequencys,values,xLabel,yLabel,title)

	def copy(self):
		
		valueName = str(self.treeWidget.currentItem().parent().data(0,0).toString())
		quantity = str(self.treeWidget.currentItem().data(0,0).toString())
		waveDirection = str(self.treeWidget.currentItem().parent().parent().parent().data(0,0).toString())
		speed = str(self.treeWidget.currentItem().parent().parent().data(0,0).toString())


		text = "%s\n%s\n%s\n%s\n%s\t%s\t%s\t%s\n" % (valueName,quantity,waveDirection,speed,"value","frequency","encounterFrequency","waveLength")
		...
		for childIndex in range(self.treeWidget.currentItem().childCount()):
			...
			text += "%f\t%f\t%f\t%f\n" % (value,frequency,encounterFrequency,waveLength)		
	...
	def exportRAW(self,exportPath=None):
		...
		if hasattr(self,'scoresFile'):		
			if exportPath == None:
				exportPath = QtGui.QFileDialog.getSaveFileName(self, 'Export file',"c:/","RAO (*.mat)")
			...
			for speed, speedResults in self.scoresFile.results.iteritems():
				for waveAngle,waveDirResult in speedResults.items():
					...
					for addedResistance,waveLength in zip(waveDirResult.addedResistance.forces,waveDirResult.addedResistance.waveLengths):
						...
						rho = 1025.0
						g = 9.81
						RAOaddresNonDim = addedResistance * 1000 / (rho * g * B**2 / Lpp)										
												
						matrix.append([waveAngle,speed,lambdaOverLpp,RAOaddresNonDim])
			...
			try:
				...
				self.parent.statusBar().showMessage('RAO exported to: %s' % exportPath)
	...
	def exportRAO(self,exportPath=None):
		...
		if hasattr(self,'scoresFile'):		
			...
			dt = [('waveDirection', 'f8'),('title', 'S20'),('nominalSpeed', 'f8'),('x', 'O8'),('yTitle', 'S20'),('y', 'O8')]
			...
			for speed, speedResults in self.scoresFile.results.iteritems():
				...
				for waveAngle,waveDirResult in speedResults.items():
					...
					rho = 1000.0
					g = 9.81
					RAOaddresNonDim = waveDirResult.addedResistance.forces * 1000 / (rho * g * B**2 / Lpp)										

					lambdaOverLpp = waveDirResult.addedResistance.waveLengths / Lpp
					RAOs[waveAngleCounter][speedCounter][0]['waveDirection'] = waveAngle			
					RAOs[waveAngleCounter][speedCounter][0]['nominalSpeed'] = speed			
					RAOs[waveAngleCounter][speedCounter][0]['x'] = lambdaOverLpp
					RAOs[waveAngleCounter][speedCounter][0]['y'] = RAOaddresNonDim
					RAOs[waveAngleCounter][speedCounter][0]['title'] = 'Added resistance'
					RAOs[waveAngleCounter][speedCounter][0]['yTitle'] = 'addedResistance'

					lambdaOverLpp = waveDirResult.verticalPlaneResponses.waveLengths / Lpp
					RAOs[waveAngleCounter][speedCounter][1]['waveDirection'] = waveAngle			
					RAOs[waveAngleCounter][speedCounter][1]['nominalSpeed'] = speed			
					RAOs[waveAngleCounter][speedCounter][1]['x'] = lambdaOverLpp
					RAOs[waveAngleCounter][speedCounter][1]['y'] = waveDirResult.verticalPlaneResponses.heaveAmplitude
					RAOs[waveAngleCounter][speedCounter][1]['title'] = 'Heave'
					RAOs[waveAngleCounter][speedCounter][1]['yTitle'] = 'heave'

					lambdaOverLpp = waveDirResult.verticalPlaneResponses.waveLengths / Lpp
					RAOs[waveAngleCounter][speedCounter][2]['waveDirection'] = waveAngle			
					RAOs[waveAngleCounter][speedCounter][2]['nominalSpeed'] = speed			
					RAOs[waveAngleCounter][speedCounter][2]['x'] = lambdaOverLpp
