...
class RAOs():
	...
	def append(self,rowIndex,columnIndex,rAO):
		...
		if newRao:
			message = "RAO ship speed:%f waveDirection:%f has been added to the ship" % (rowIndex,columnIndex)
		else:
			message = "RAO shipSpeed:%f waveDirection:%f has NOT been added to the ship, since the place was already taken" % (rowIndex,columnIndex)
		log.info(message)
...
class RAO():
	...
	def __init__(self,shipSpeed,waveDirection,frequencies=None,responses=None):
	...
	def calculateAddedResistanceInIrregularWaves(self,rho,B,Lpp,Tzs):
		...
		for Tz in Tzs:
			self.addedResistanceIrregularWaves.append(irregularSeaClass(self,rho,B,Lpp,Tz))
	...
	def calculateAddedResistanceInRegularWaves(self,rho,B,Lpp,Tps):
		...
		for Tp in Tps:
			self.addedResistanceRegularWaves.append(regularSeaClass(self,rho,B,Lpp,Tp))
...
class regularSeaClass():
	...
	def calculateAddedResistance(self,rho,B,Lpp,Tp):
		...
		# Raw / H^2:
		RawTmp = RAOnonDimExt*(4*rho*g*B**2/Lpp) # Raw/(wave ampl ^ 2)
