...
class Scores1Results(object):
	...
	def getResponseSpectra(self,s,itemSeparator):
		...
		searchResult = re.search("RESPONSE SPECTRA(.*?)%s(.*)" % itemSeparator,s,re.DOTALL)
		...
		searchResult = re.search("0SPEED=(.*?) KNOTS   WAVE ANGLE=(.*?) DEG",lines[1],re.DOTALL)
		...
		for line in lines[5:]:
			...
			for nameTag in nameTags:						
				...
				self.waveSpectras[spectrum].responses[speed][waveDirection][nameTag] = value
				colCount+=1
