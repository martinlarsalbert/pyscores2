...
class Widget(QtGui.QWidget):
	def __init__(self,parent=None):
		...
		zcgDescription = QtGui.QLabel('(CG meassured from WL, positive upwards)')
		partOfCriticalRollDamping = QtGui.QLabel('Part of critical roll damping [-]')
		...
		self.projectNameEdit = QtGui.QLineEdit(parent.scores2Indata.projectName)
		self.lppEdit = QtGui.QLineEdit(str(parent.scores2Indata.lpp))
		self.displacementEdit = QtGui.QLineEdit(str(parent.scores2Indata.displacement))
		...
		self.partOfCriticalRollDampingEdit = QtGui.QLineEdit(str(parent.scores2Indata.partOfCriticalRollDamping))
		...
		self.speedIncrementEdit = QtGui.QLineEdit(str(parent.scores2Indata.speedIncrement))
		self.waveDirectionMinEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveDirectionMin))
		self.waveDirectionMaxEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveDirectionMax))
		self.waveDirectionIncrementEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveDirectionIncrement))
		self.waveDirectionMinEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveDirectionMin))
		self.waveDirectionMaxEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveDirectionMax))
		self.waveDirectionIncrementEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveDirectionIncrement))
		self.waveFrequenciesMinEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveFrequenciesMin))
		self.waveFrequenciesMaxEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveFrequenciesMax))
		self.waveFrequenciesIncrementEdit = QtGui.QLineEdit(str(parent.scores2Indata.waveFrequenciesIncrement))
	...
	def displayData(self):
		...
		rowCounter = 0
		for b,cScores,t in zip(self.parent.scores2Indata.bs,self.parent.scores2Indata.cScores,self.parent.scores2Indata.ts):
		...
		self.displacementEdit.setText(str(self.parent.scores2Indata.displacement))
		...
		self.partOfCriticalRollDampingEdit.setText(str(self.parent.scores2Indata.partOfCriticalRollDamping))
		...
		self.speedIncrementEdit.setText(str(self.parent.scores2Indata.speedIncrement)) 
		self.waveDirectionMinEdit.setText(str(self.parent.scores2Indata.waveDirectionMin))
		self.waveDirectionMaxEdit.setText(str(self.parent.scores2Indata.waveDirectionMax))
		self.waveDirectionIncrementEdit.setText(str(self.parent.scores2Indata.waveDirectionIncrement))
		self.waveFrequenciesMinEdit.setText(str(self.parent.scores2Indata.waveFrequenciesMin))
		self.waveFrequenciesMaxEdit.setText(str(self.parent.scores2Indata.waveFrequenciesMax))
		self.waveFrequenciesIncrementEdit.setText(str(self.parent.scores2Indata.waveFrequenciesIncrement))
	...
	def getData(self):
		...
		try:
			...
			if abs(rho) < 10:
				...
				messageBox.showMessage("rho is to small. It should be expressed in [kg/m3]")
		...
		try:
			...
			tempIndata.zcg = 0.0 
		try:
			tempIndata.partOfCriticalRollDamping	= float(self.partOfCriticalRollDampingEdit.text())
		except:
		...
		try:
			tempIndata.waveDirectionMin			= float(self.waveDirectionMinEdit.text())
		except:
			tempIndata.waveDirectionMin = 0.0
		try:
			tempIndata.waveDirectionMax			= float(self.waveDirectionMaxEdit.text())
		except:
			tempIndata.waveDirectionMax = 0.0
		try:
			tempIndata.waveDirectionIncrement	= float(self.waveDirectionIncrementEdit.text())
		except:
			tempIndata.waveDirectionIncrement = 0.0
		
		try:
			tempIndata.waveFrequenciesMin			= float(self.waveFrequenciesMinEdit.text())
		except:
			tempIndata.waveFrequenciesMin = 0.0
		try:
			tempIndata.waveFrequenciesMax			= float(self.waveFrequenciesMaxEdit.text())
		except:
			tempIndata.waveFrequenciesMax = 0.0
		try:
			tempIndata.waveFrequenciesIncrement	= float(self.waveFrequenciesIncrementEdit.text())
		except:
	...
	def updateIndata(self,tempIndata):
		...
		for varname in variables:
			if hasattr(tempIndata, varname):
				if self.parent.scores2Indata.__dict__[varname] != tempIndata.__dict__[varname]:
					same = False
					break		
		if not same:
...
class MainWindow(QtGui.QMainWindow):
	...
	def initUI(self):
		...
		runScores2Action = QtGui.QAction(QtGui.QIcon('run.jpg'),'Run ScoresII', self)        
	...
	def open(self):
		...
		fname = QtGui.QFileDialog.getOpenFileName(self, 'Open file',resentFile,"ScoresII in (*.in)")
		...
		if os.path.exists(fname):
			...
			if tempIndata.runOptions["ID"] == 0:
				...
				for Hs,Tp in zip(tempIndata.H33s,tempIndata.Tps):
					newSpectrum = waveSpectrum.JonswapSpectrum("NoName",Hs,Tp)
	...
	def save(self):
		...
		if self.savePath == "":
			self.savePath = str(QtGui.QFileDialog.getSaveFileName(self, 'Save file',str(self.scores2Indata.projectName) + ".in","ScoresII in (*.in)"))
		
		self.scores2Indata.save(self.savePath,self.waveSpectrumWidget.waveSpectrums)
	...
	def saveAs(self,indataPath):
		...
		self.savePath = QtGui.QFileDialog.getSaveFileName(self, 'Save file as',"c:/","ScoresII in (*.in)")
		self.scores2Indata.save(self.savePath,self.waveSpectrumWidget.waveSpectrums)
	...
	def runScores2(self):
		...
		if self.w.tv.item(0,0) != None:
			...
			calculation = runScores2.scores2CalculationClass(tempIndataPath,os.path.abspath(""))
			...
			if successful:
				self.statusBar().showMessage('The ScoresII calculation completed')
				...
				if  len(self.resultWidget.scoresFile.irregularResults) > 0:
					...
					wantedSpectrumID = int(self.scores2Indata.runOptions["ID"].getValue())
					counter = 0
					counter2 = 0
					for waveSpectrum in self.waveSpectrumWidget.waveSpectrums:
						...
						if waveSpectrum.scoresFileID == wantedSpectrumID:
							if counter <= 5: #Max 5 spectra alowed in scoresII
								spectrumIndex = self.resultWidget.scoresFile.spectrumIndexList[counter]
				...
				messageBox = QtGui.QErrorMessage(self)
				messageBox.showMessage("The ScoresII calculation was not successful! Error No.%s %s" % (errorCode,errorDescription))
	...
	def importXML(self):
		...
		fname = QtGui.QFileDialog.getOpenFileName(self, 'Open file','',"Hydrostatics (*.xml)")
		...
		if os.path.exists(fname):
			...
			updatedTempIndata = hydrostatics.convertToScores2Indata(tempIndata = tempIndata,conditionName = conditionName)	
		...
		for optionID in self.parent.scores2Indata.runOptionTags:
			...
			if option.alternatives:
				...
				for alternative in option.alternatives:
					radioButton = QtGui.QRadioButton(alternative.description,self)
					radioButton.setDown(alternative.getValue())
					
					self.grid.addWidget(radioButton,rowCounter,1+columnCounter,1,1)
	...
	def closeEvent(self,QCloseEvent):
		for optionID in self.parent.scores2Indata.runOptionTags:
			...
			if type(inputItem) == type(QtGui.QLineEdit()):
				self.parent.scores2Indata.runOptions[optionID].setValue(int(inputItem.text()))
			elif type(inputItem) == type(QtGui.QButtonGroup()):
				counter = 0
				for radioButton in inputItem.buttons():
					if radioButton.isChecked():
						self.parent.scores2Indata.runOptions[optionID].setValue(counter)
						break
					counter +=1					
