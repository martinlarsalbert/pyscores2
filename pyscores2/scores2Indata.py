...
class Scores2Indata():
	def __init__(self):
		...
		self.runOptions["IA"].addOption(description = r"Hullform given at segment midpoint")
		self.runOptions["IA"].addOption(description = r"Hullform given at segment endpoint")
		self.runOptions["IA"].setValue(1)
		
		self.runOptions["IB"] = RunOption(description = r"Mass & Moment")
		self.runOptions["IB"].addOption(description = r"Motion only, use ship total mass properties")
		self.runOptions["IB"].addOption(description = r"Motion only, use mass dist.")
		...
		self.runOptions["IE"].addOption(description = r"Vertical and Lateral plane")
		...
		self.runOptions["IF"].addOption(description = r"Cos-square wave spreading")
		...
		self.runOptions["IH"].addOption(description = r"Calc. and print out closure results")
		...
		self.runOptions["N"] = RunOption(description = r"No. of equally spaced segments along the ship")
		self.runOptions["N"].setValue(20)

		self.runOptions["IK"] = RunOption(description = r"Roll damping")
		self.runOptions["IK"].addOption(description = r"Input empirical total damping")
		self.runOptions["IK"].addOption(description = r"Input empirical nonlinear damping")
		self.runOptions["IK"].addOption(description = r"Use program calculated nonlinear damping")
		self.runOptions["IK"].setValue(0)

		self.runOptions["IL"] = RunOption(description = r"Use 3-D Damping correction factors heave & pitch")
		self.runOptions["IL"].addOption(description = r"No correction")
		self.runOptions["IL"].addOption(description = r"Use 3-D correction factors")
		self.runOptions["IL"].setValue(0)

		self.runOptions["IM"] = RunOption(description = r"Total number of sections to be analyzed by close-fitted method")
	...
	def open(self,indataPath):
		...
		#..see manual...
		rowCounter+=1
		values = lines[rowCounter].split() #This one should probably be devided into a linar and one nonlinear part (remains to be implemented)
	...
	def save(self,indataPath,waveSpectrums=None):
		...
		for accelerationPoint in self.accelerationPoints:
			file.write("%10.5f%10.5f%10.5f\n" % (accelerationPoint["XAC"],accelerationPoint["YAC"],accelerationPoint["ZAC"]))
		...
		if self.runOptions["ID"].getValue() == 0:
			...
			waveFrequenciesMin			= 2*np.pi*self.g / (self.waveFrequenciesMin)**2
			waveFrequenciesMax			= 2*np.pi*self.g / (self.waveFrequenciesMax)**2
			waveFrequenciesIncrement	= 2*np.pi*self.g / (self.waveFrequenciesIncrement)**2
		...
		file.write("%-10.1f%-10.1f%-10.1f\n" % (waveDirectionMinScores,waveDirectionMaxScores,self.waveDirectionIncrement))
	...
	def setValue(self,value = None):
		if len(self.alternatives) > 0:
			if type(value) == type(int()):
				...
				if subAlternative >= 0 and subAlternative < len(self.alternatives):
					for alternative in self.alternatives:
						alternative.value = False
											
					self.alternatives[subAlternative].value = True
				else:
					raise ValueError("Subalternative %i does not exist" % subAlternative)								
			else:
