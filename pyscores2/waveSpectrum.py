...
class WaveSpectrum(object):
		...
		for speed in sorted(scoresFile.results.iterkeys()):
			...
			for waveAngle in sorted(scoresFile.results[speed].iterkeys()):
				...
				if result.lateralPlaneResponses.frequencies == None:
					self.results[speed][waveAngle]['lateralPlaneResponses']['sway'] = IrregularResult(0,0,0,0)
					self.results[speed][waveAngle]['lateralPlaneResponses']['yaw']	= IrregularResult(0,0,0,0)
					self.results[speed][waveAngle]['lateralPlaneResponses']['roll'] = IrregularResult(0,0,0,0)
				else:
					self.results[speed][waveAngle]['lateralPlaneResponses']['sway'] = self.calculateLinearResponse(result.lateralPlaneResponses.frequencies,result.lateralPlaneResponses.swayAmplitude)
					self.results[speed][waveAngle]['lateralPlaneResponses']['yaw'] = self.calculateLinearResponse(result.lateralPlaneResponses.frequencies,result.lateralPlaneResponses.yawAmplitude)
					self.results[speed][waveAngle]['lateralPlaneResponses']['roll'] = self.calculateLinearResponse(result.lateralPlaneResponses.frequencies,result.lateralPlaneResponses.rollAmplitude)				
				...
				if result.verticalPlaneResponses.frequencies == None:
					self.results[speed][waveAngle]['verticalPlaneResponses']['surge']= IrregularResult(0,0,0,0)
					self.results[speed][waveAngle]['verticalPlaneResponses']['heave']= IrregularResult(0,0,0,0)
					self.results[speed][waveAngle]['verticalPlaneResponses']['pitch']= IrregularResult(0,0,0,0)
				else:
					self.results[speed][waveAngle]['verticalPlaneResponses']['surge'] = self.calculateLinearResponse(result.verticalPlaneResponses.frequencies,result.verticalPlaneResponses.surgeAmplitude)
					self.results[speed][waveAngle]['verticalPlaneResponses']['heave'] = self.calculateLinearResponse(result.verticalPlaneResponses.frequencies,result.verticalPlaneResponses.heaveAmplitude)
					self.results[speed][waveAngle]['verticalPlaneResponses']['pitch'] = self.calculateLinearResponse(result.verticalPlaneResponses.frequencies,result.verticalPlaneResponses.pitchAmplitude)				
				...
				if hasattr(result.pointAccelerations,'frequencies'):
					...
					if result.pointAccelerations.frequencies == None:
						self.results[speed][waveAngle]['pointAccelerations']['x']= IrregularResult(0,0,0,0)
						self.results[speed][waveAngle]['pointAccelerations']['y']= IrregularResult(0,0,0,0)
						self.results[speed][waveAngle]['pointAccelerations']['z']= IrregularResult(0,0,0,0)
					else:
						self.results[speed][waveAngle]['pointAccelerations']['x'] = self.calculateLinearResponse(result.pointAccelerations.frequencies,result.pointAccelerations.longitudinalAmplitude)
						self.results[speed][waveAngle]['pointAccelerations']['y'] = self.calculateLinearResponse(result.pointAccelerations.frequencies,result.pointAccelerations.lateralAmplitude)
						self.results[speed][waveAngle]['pointAccelerations']['z'] = self.calculateLinearResponse(result.pointAccelerations.frequencies,result.pointAccelerations.verticalAmplitude)	
	...
	def calculateLinearResponse(self,RAOFrequencies,RAO):
		...
		SPrim = self.get()
		cutIndex = np.logical_and(min(RAOFrequencies) <= self.frequencies,max(RAOFrequencies) >= self.frequencies)
...
class ITTCSpectrum(WaveSpectrum):
	...
	def get(self,frequencies=None):
		...
		self.S = self.A*self.B*self.frequencies**(-5)*np.exp(-self.B*self.frequencies**(-4))
...
class JonswapSpectrum(WaveSpectrum):
	...
	def get(self,frequencies=None):
		...
		for frequency,s in zip(self.frequencies,ss):
			...
			fct = np.exp(-1/2/sigma**2*(0.159*frequency*self.Tp-1)**2)
